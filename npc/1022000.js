  ...
  if (status == 0) {
    if (cm.getJob() == 0) {
      if (cm.getPlayerStat("LVL") >= 10 && cm.getJob() == 0) {
        cm.sendNext("So you decided to become a #r劍士#k?");
      } else {
    ...
    cm.sendYesNo("Do you want to become a #r劍士#k?");
  } else if (status == 3) {
    if (cm.getJob() == 0) {
    ...
    cm.sendNextPrev("You may be ready to take the next step as a #r狂戰士#k, #r見習騎士#k or #r槍騎兵#k.")
  } else if (status == 12) {
