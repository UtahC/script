function enter(pi) {
 try {
	...
	pi.warp(pi.getMapId(),(pi.getPortal().getName().startsWith("pt9") ? 14 : (pi.getPortal().getId() + 3)));
	pi.playerMessage(-1, "Correct combination!");
	pi.getMap().changeEnvironment("an" + pi.getPortal().getName().substring(2, 4), 2);
    } else {
	pi.warp(pi.getMapId(), (pi.getPortal().getName().startsWith("pt0") ? 13 : (pi.getPortal().getId() - 3)));
